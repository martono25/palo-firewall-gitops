...
variable "folder_interfaces" {
  description = "Folder-scope `$`-interface VARIABLES, from catalog/interfaces.yaml via `fwgitops folder-interfaces`. Separate from `interfaces` because Terraform REPLACES a variable set by two auto-tfvars files instead of merging; main.tf merges them explicitly."
  type = map(object({
    name = string
    # Exactly one of folder/device is non-null (the provider enforces "exactly
    # one of device, folder, snippet"). A firewall is the last level of the SCM
