...
def test_snapshot_requires_exactly_one_scope(capsys):
    ...
    assert main(["snapshot", "InterfaceRequest", "prod-edge",
                 "--device", "007955000902404", "--out", "/tmp/x.json"]) == 1
...
folders:
  ...
  prod-edge:
    ...
    targetable: true
    devices:
      "007955000902404": {display_name: fw-a, model: PA-VM, targetable: true}
  GitOps:
    children: []
    targetable: true
"""
...
def test_a_push_record_is_matched_by_SCOPE_not_by_scm_address(tmp_path):
    ...
    rec.write_text(json.dumps({
        "scope_dir": "device-007955000902404",   # the Terraform root
        "folder": "007955000902404",             # the SCM address
