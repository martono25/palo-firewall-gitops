...
def _ifcat():
    return InterfaceCatalog.from_dict({"interfaces": {
        "local": {"folder": "$eth-local", "devices": {"007955000902404": "ethernet1/4"}},
        "internet": {"folder": "$eth-internet", "devices": {"007955000902404": "ethernet1/3"}},
    }})
...
def test_the_same_role_resolves_differently_per_scope():
    ...
    assert cat.resolve("local", device="007955000902404") == "ethernet1/4"
...
def test_a_role_is_resolved_against_the_intents_actual_target():
    ...
    at_device = load_intent(_doc(environment=None, device="007955000902404"),
                            interface_catalog=_ifcat(), folder_hierarchy=h)
