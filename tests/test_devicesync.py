...
DEV = {"serial_number": "007955000902404", "name": "007955000902404",
...
def test_a_device_behind_its_folder_is_flagged():
    ...
    r = compare([DEV], {"007955000902404": 68}, {"prod-edge": 70})[0]
...
def test_a_device_running_the_newest_version_is_clean():
    r = compare([DEV], {"007955000902404": 70}, {"prod-edge": 70})[0]
...
def test_a_missing_version_is_UNKNOWN_not_assumed_fine():
    ...
    assert compare([DEV], {"007955000902404": 70}, {})[0].state == UNKNOWN
...
def test_the_command_exits_2_when_a_device_is_behind(capsys):
    rc = run_device_sync(session=_Session(
        [DEV], [{"device": "007955000902404", "version": 68}], [{"id": 70}]))
    assert rc == 2
...
def test_the_command_exits_0_when_everything_is_current(capsys):
    rc = run_device_sync(session=_Session(
        [DEV], [{"device": "007955000902404", "version": 70}], [{"id": 70}]))
    assert rc == 0
...
def test_the_command_exits_0_for_first_push_pending_but_says_so(capsys):
    ...
    dev = {**DEV, "is_first_push_done": False}
    rc = run_device_sync(session=_Session(
        [dev], [{"device": "007955000902404", "version": 72}], [{"id": 72}]))
...
def test_a_device_scope_row_defines_at_device_not_in_a_folder(tmp_path, capsys):
    ...
    snap.write_text(json.dumps([{
        ...
        "device": "007955000902404", "scope": "device:007955000902404",
        "layer3": {}}]))
...
def test_only_the_scopes_the_snapshot_COVERS_are_compared(tmp_path, capsys):
    ...
    declared = {
        ("device:007955000902404", "ethernet1/2"): {"name": "ethernet1/2"},
        ("prod-edge", "dmz"): {"name": "dmz"},
    }
    actual = [ActualObject(kind="InterfaceRequest", folder="device:007955000902404",
                           name="ethernet1/2", scope="device:007955000902404",
