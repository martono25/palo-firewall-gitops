...
The roots are copies: `terraform/prod-edge/variables.tf` and every
`terraform/device-<serial>/variables.tf` restate the module's `variable`
...
def _scaffolded_device_root() -> Path:
    """A DEVICE root generated by `scaffold-root`, outside the repo.

    From 2026-09-14 no firewall was registered, so the repository held no
    device root and every parametrized test below lost its device case —
    silently, because a parametrize over zero roots of a kind is not a failure.
    A device root is generated from a DIFFERENT template path than a folder
    root, so dropping it is a real loss of coverage, not a tidy-up.

    Generating one keeps it, and tests the generator the next firewall will
    actually be onboarded with rather than a hand-kept copy.
    """
    import io
    import tempfile

    from fwgitops.cli import run_scaffold_root
    from onboarded_catalog import EXAMPLE_SERIAL

    out = Path(tempfile.mkdtemp(prefix="fwgitops-scaffold-"))
    rc = run_scaffold_root(out, device=EXAMPLE_SERIAL, device_folder="prod-edge",
                           module_dir=MODULE, out=io.StringIO(), err=io.StringIO())
    assert rc == 0, "scaffold-root must be able to generate a device root"
    (root,) = [p for p in out.iterdir() if p.is_dir()]
    return root


def _roots() -> list:
    """Every root that calls the security_folder module — the repo's, plus one
    scaffolded device root while no firewall is registered."""
    found = []
    for main in sorted(TF_DIR.glob("*/main.tf")):
        if 'source = "../modules/security_folder"' in main.read_text():
            found.append(main.parent)
    if not any(r.name.startswith("device-") for r in found):
        found.append(_scaffolded_device_root())
    return found
...
def test_there_is_at_least_one_root():
    ...
    # distinct object, so it must not share state with its folder. EVERY
    # registered firewall, read from the real catalog rather than a literal
    # serial — the serial has changed five times.
    import yaml
    ...
    on_disk = {p.parent.name for p in TF_DIR.glob("*/main.tf")}
    for serial in sorted(h.targetable_devices):
        assert Scope("device", serial).dirname in on_disk, (
            f"firewall {serial} is targetable but has no Terraform root — "
            f"`fwgitops scaffold-root --device {serial}`")
...
def _compiled_pairs():
    ...
    for path in sorted((root / "intent").rglob("*.yaml")):
        # Discovery skips `*.example.*`, exactly as `fwgitops compile` does:
        # "the real tree" means what the compiler compiles. Reading examples too
        # made this depend on a firewall being registered, because the
        # InterfaceRequest example names one.
